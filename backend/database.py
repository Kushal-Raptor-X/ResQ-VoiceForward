...
async def get_system_insights() -> dict:
    """Supervisor dashboard — MongoDB aggregations (falls back to in-memory)."""
    db = get_db()
    if db is not None:
        try:
            data = await CallModel(db).supervisor_insights()
    ...
    for d in store:
        ...
        action = (d.get("operator_action") or "").lower()
        for p in d.get("phrases") or []:
            b = by_phrase.setdefault(p, {"phrase": p, "hits": 0, "n": 0})
            b["n"] += 1
            if outcome == "resolved" and action == "accepted":
                b["hits"] += 1
    top = sorted(
        (
            {
                "phrase": v["phrase"],
                "success_rate": round(v["hits"] / max(v["n"], 1), 4),
                "samples": v["n"],
            }
            for v in by_phrase.values()
            if v["n"] >= 1
        ),
        key=lambda x: -x["success_rate"],
    )[:8]
    return {
        "top_effective_phrases": top,
        "high_risk_patterns": [],
        "resource_gaps": [],
        "source": "in-memory",
    }
